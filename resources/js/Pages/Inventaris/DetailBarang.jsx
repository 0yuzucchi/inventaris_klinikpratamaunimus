...
import {
    ArrowLeftIcon, PencilSquareIcon, PhotoIcon, CalendarDaysIcon, CurrencyDollarIcon,
    ArchiveBoxIcon, MapPinIcon, UserCircleIcon, QrCodeIcon, TagIcon,
    ClipboardDocumentIcon, ArrowDownTrayIcon, BeakerIcon, BuildingLibraryIcon
} from '@heroicons/react/24/solid';

// Helper penanganan null
const formatDate = (dateString) => {
    if (!dateString) return 'N/A';
    try {
        return new Date(dateString).toLocaleDateString('id-ID', { year: 'numeric', month: 'long', day: 'numeric' });
...
const DetailItem = ({ icon, label, children, className = '' }) => (
    <div className={`py-4 flex items-start gap-4 ${className}`}>
        ...
        <div className="flex-1">
            <p className="text-sm font-medium text-slate-500 dark:text-slate-400">{label}</p>
            <div className="text-base font-semibold text-slate-800 dark:text-white mt-1">
                {children ?? <span className="text-slate-400 font-normal italic">N/A</span>}
            </div>
        </div>
    </div>
);
...
export default function DetailBarang({ auth, inventari }) {
    ...
    const handleDownloadQR = () => {
        ...
        const link = document.createElement('a');
        link.href = url;
        link.download = `QR-${(item.display_nomor_barang || 'KODE')}.svg`;
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
        URL.revokeObjectURL(url);
    ...
    const PageContent = (
        <div className="flex-1 w-full bg-white dark:bg-slate-800">
            <Head title={`Detail - ${item.display_nama_barang}`} />
            <div className="container mx-auto px-4 sm:px-6 lg:px-8 h-full">
                <div className="grid grid-cols-1 lg:grid-cols-5 min-h-full">
                    {/* === KOLOM KIRI: VISUAL === */}
                    <aside className="lg:col-span-2 lg:border-r border-slate-200 dark:border-slate-700 p-6 sm:p-8 flex flex-col items-center">
                        <div className="w-full max-w-sm mx-auto text-center space-y-8">
                            {item.foto ? (
                                <img
                                    src={item.foto.startsWith('http') ? item.foto : `/storage/${item.foto}`}
                                    alt={item.display_nama_barang}
                                    className="w-full h-64 object-cover rounded-xl shadow-lg border border-slate-200 dark:border-slate-700"
                                    onError={(e) => { e.target.onerror = null; e.target.src = 'https://via.placeholder.com/400x300?text=Gambar+Tidak+Ditemukan'; }}
                                />
                            ) : (
                                <div className="w-full h-64 bg-slate-200 dark:bg-slate-700 rounded-xl flex items-center justify-center text-slate-400 dark:text-slate-500">
                                    <PhotoIcon className="w-24 h-24" />
                            ...
                            <div className="w-full p-6 bg-slate-50 dark:bg-slate-900/50 rounded-xl border border-slate-200 dark:border-slate-700 text-center">
                                <h3 className="text-sm font-bold text-slate-600 dark:text-slate-300 uppercase tracking-wider mb-3">QR Code Unit</h3>
                                <div ref={qrCodeRef} className="p-3 bg-white inline-block rounded-lg shadow-inner">
                                    <QRCodeSVG value={route('inventaris.show', item.id, { absolute: true })} size={160} />
                                </div>
                                <button onClick={handleDownloadQR} className="mt-4 w-full inline-flex items-center justify-center gap-2 px-3 py-2 bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded-lg text-sm font-semibold text-slate-700 dark:text-slate-200 hover:bg-slate-50 transition-colors">
                                    <ArrowDownTrayIcon className="w-4 h-4" /> Unduh QR Code
                                </button>
                            </div>
                        </div>
                    </aside>
                    ...
                    <main className="lg:col-span-3 p-6 sm:p-8">
                        <div className="mb-6 pb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-200 dark:border-slate-700">
                            {auth.user ? (
                                <Link href={route('inventaris.index')} className="inline-flex items-center gap-2 px-4 py-2 bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded-lg text-sm font-semibold text-slate-700 dark:text-slate-200 hover:bg-slate-50 transition-colors">
                            ...
                            <div className="flex items-center gap-3 w-full sm:w-auto">
                                {auth.user && (
                                    <>
                                        <a href={route('inventaris.generateLabel', { inventari: item.id })} target="_blank" className="flex-1 sm:flex-initial justify-center inline-flex items-center gap-2 px-4 py-2 bg-green-100 text-green-700 border border-green-300 rounded-lg text-sm font-semibold hover:bg-green-200 transition-colors">
                                            <QrCodeIcon className="w-4 h-4" /> Cetak Label
                                        </a>
                                        <Link href={route('inventaris.edit', item.id)} className="flex-1 sm:flex-initial justify-center inline-flex items-center gap-2 px-4 py-2 bg-blue-600 rounded-lg text-sm font-bold text-white hover:bg-blue-700 transition-transform">
                                            <PencilSquareIcon className="w-4 h-4" /> Edit Data
                                        </Link>
                                    </>
                                )}
                            </div>
                        ...
                        <div className="mb-8">
                            ...
                            <p className="text-lg text-slate-500 dark:text-slate-400 mt-1">
                                No. Inventaris: <span className="font-bold text-blue-600 dark:text-blue-400">{item.display_nomor_barang}</span>
                            </p>
                        </div>

                        <div className="divide-y divide-slate-200 dark:divide-slate-700">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8">
                                <DetailItem icon={MapPinIcon} label="Lokasi Unit">
                                    <p>{item.display_ruangan?.toUpperCase()}</p>
                                    <p className="text-sm text-slate-500 font-medium">No. Ruang: {item.display_nomor_ruang}</p>
                                </DetailItem>

                                <DetailItem icon={ArchiveBoxIcon} label="Status & Jumlah Stok">
                                    <div className="flex gap-4">
                                        <span>Total: <span className="text-blue-600">{item.jumlah}</span></span>
                                        <span>Rusak: <span className="text-red-600">{item.jumlah_rusak}</span></span>
                                    </div>
                                </DetailItem>

                                <DetailItem icon={TagIcon} label="Kode Barang">
                                    {item.kode_barang?.toUpperCase() ?? 'N/A'}
                                </DetailItem>

                                <DetailItem icon={CurrencyDollarIcon} label="Harga Satuan">
                                    {formatCurrency(item.harga)}
                                </DetailItem>

                                <DetailItem icon={CalendarDaysIcon} label="Tanggal Perolehan">
                                    {formatDate(item.tanggal_masuk)}
                                </DetailItem>

                                {/* KOLOM TERPISAH UNTUK ASAL PEROLEHAN */}
                                <DetailItem icon={BuildingLibraryIcon} label="Asal Perolehan">
                                    {item.display_asal?.toUpperCase() ?? 'N/A'}
                                </DetailItem>

                                <DetailItem icon={BeakerIcon} label="Jenis Perawatan" className="md:col-span-2">
                                    {item.display_jenis_perawatan?.toUpperCase()}
                                </DetailItem>
                            </div>

                            <DetailItem icon={UserCircleIcon} label="Diunggah Oleh" className="border-t">
                                {item.nama_pengunggah || 'N/A'}
                            </DetailItem>

                            <DetailItem icon={ClipboardDocumentIcon} label="Spesifikasi / Catatan" className="border-t">
                                <p className="whitespace-pre-wrap leading-relaxed text-slate-600 dark:text-slate-400 font-normal">
                                    {item.spesifikasi || 'Tidak ada spesifikasi khusus untuk unit ini.'}
                                </p>
                            </DetailItem>
                        </div>
                    </main>
    ...
    if (auth.user) {
        return (
            <AuthenticatedLayout user={auth.user} header="Detail Inventaris">
                {PageContent}
            </AuthenticatedLayout>
        );
    }
    ...
    return (
        <div className="min-h-screen bg-slate-50 dark:bg-slate-900">
            <nav className="bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 py-4 shadow-sm">
                <div className="container mx-auto px-6 font-bold text-slate-800 dark:text-white flex items-center gap-2">
                    <div className="w-8 h-8 bg-blue-600 rounded flex items-center justify-center text-white">I</div>
                    Sistem Inventaris
                </div>
            </nav>
            <div className="py-6">
