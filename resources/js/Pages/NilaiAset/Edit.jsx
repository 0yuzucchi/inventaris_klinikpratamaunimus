...
// --- KOMPONEN UI (Sama seperti Create.jsx) ---
        ...
        <div className="relative">
            ...
            <input
                ...
                className={`
                    ...
                    focus:border-blue-500 focus:ring focus:ring-blue-500 dark:focus:ring-blue-600 focus:ring-opacity-50
...
const TextArea = ({ name, label, value, onChange, error, ...props }) => (
    <InputGroup label={label} htmlFor={name}>
        <textarea
            ...
            value={value || ''}
            onChange={onChange}
            className="block w-full rounded-md bg-slate-50 dark:bg-slate-700 border-transparent dark:border-slate-600 text-slate-900 dark:text-white shadow-sm px-4 py-2 focus:border-blue-500 focus:ring focus:ring-blue-500 dark:focus:ring-blue-600 focus:ring-opacity-50"
            {...props}
        />
        {error && <p className="text-red-500 text-xs mt-1">{error}</p>}
    </InputGroup>
...
const RelationalDropdown = ({ label, value, options = [], onSelect, error, required, placeholder = "Pilih..." }) => {
    ...
    // Convert value to string for loose comparison (id dari DB integer, value form mungkin string)
                ...
                {isOpen && (
                    <div className="absolute z-[100] mt-2 min-w-full w-auto max-w-[90vw] sm:max-w-md overflow-hidden rounded-xl bg-white dark:bg-slate-800 shadow-2xl ring-1 ring-black ring-opacity-5 animate-in fade-in zoom-in-95 duration-100 origin-top">
                        <div className="sticky top-0 z-10 border-b border-slate-100 dark:border-slate-700 bg-white/95 dark:bg-slate-800/95 backdrop-blur-sm p-3">
                            <input
                                type="text"
                                className="block w-full rounded-lg border-none bg-slate-100 dark:bg-slate-900 py-2 pl-4 text-sm text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 transition-all placeholder:text-slate-400"
                                placeholder="Cari..."
                                value={searchTerm}
                                            ...
                                            >
                                                {isComplex ? (
                                                    <>
                                                        ...
                                                        <div className="flex-1 min-w-0">
                                                            <div className="flex justify-between items-start">
                                                                <span className={`text-sm font-bold pr-2 whitespace-normal break-words leading-tight ${isSelected ? 'text-blue-800 dark:text-blue-400' : 'text-slate-800 dark:text-white'}`}>{opt.nama_barang}</span>
                                                                {isSelected && <svg className="h-5 w-5 text-blue-600 flex-shrink-0" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" /></svg>}
                                                            </div>
                                                    ...
                                                    <div className="flex-1 flex justify-between items-center">
                                                        <span className={`text-sm font-medium ${isSelected ? 'text-blue-700 dark:text-blue-400' : 'text-slate-700 dark:text-slate-200'}`}>{opt.label}</span>
                                                        {isSelected && <svg className="h-5 w-5 text-blue-600" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" /></svg>}
                                                    </div>
                                                )}
                                            </button>
                                        </li>
...
export default function Edit({ auth, nilaiAset, inventarisList }) {

    // Inisialisasi Form dengan data yang ada
    const { data, setData, put, processing, errors } = useForm({
        inventaris_id: nilaiAset.inventaris_id || '',
        tanggal_perolehan: nilaiAset.tanggal_perolehan || '',
        keterangan: nilaiAset.keterangan || '',

        // Convert number ke string agar input berfungsi normal (bisa dihapus)
        harga_perolehan_awal: nilaiAset.harga_perolehan_awal ? String(nilaiAset.harga_perolehan_awal) : '',
        penambahan: nilaiAset.penambahan ? String(nilaiAset.penambahan) : '',
        pengurangan: nilaiAset.pengurangan ? String(nilaiAset.pengurangan) : '',
        tarif_penyusutan: nilaiAset.tarif_penyusutan ? String(nilaiAset.tarif_penyusutan) : '25',
    });

    const [simulasi, setSimulasi] = useState([]);
    const [totalBasis, setTotalBasis] = useState(0);

    // --- LOGIC SIMULASI JS (Sama seperti Create) ---
    useEffect(() => {
    ...
    const submit = (e) => {
        e.preventDefault();
        // Menggunakan method PUT ke route update
        put(route('nilai-aset.update', nilaiAset.id));
    };

    const handleSelectInventaris = (id) => {
        const item = inventarisList.find(i => i.id == id);
        // Pada Edit, jika user mengubah barang, kita update field terkait
        setData(prev => ({
            ...prev,
            inventaris_id: id,
            harga_perolehan_awal: item ? String(parseInt(item.harga_referensi)) : prev.harga_perolehan_awal,
            tanggal_perolehan: item && item.tanggal_referensi ? item.tanggal_referensi.split('T')[0] : prev.tanggal_perolehan,
            keterangan: item ? item.nama_barang : prev.keterangan
        }));
    };
    ...
    return (
        <AuthenticatedLayout
            user={auth.user}
            header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">Edit Data Aset</h2>}
        >
                                    ...
                                    <TextInput
                                        ...
                                        value={
                                            data.tanggal_perolehan
                                                ? data.tanggal_perolehan.includes('T')
                                                    ? data.tanggal_perolehan.split('T')[0]
                                                    : data.tanggal_perolehan
                                                : ''
                                        }
