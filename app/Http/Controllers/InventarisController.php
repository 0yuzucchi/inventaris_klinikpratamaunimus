...
//         // --- Header Utama --- //
//         $logoUrl = 'https://tnrkvhyahgvlvepjccvq.supabase.co/storage/v1/object/public/inventaris-fotos/aset/logo_klinik.png';
...
class InventarisController extends Controller
{
    public function index(): InertiaResponse
    {
        $inventaris = Inventaris::with(['masterBarang.kategori', 'ruangan', 'asalPerolehan', 'user', 'jenisPerawatan']) // Tambahkan .kategori di masterBarang
            ->latest()
            ->get();

        // Grouping berdasarkan display_nama_barang (hasil fallback)
        $grouped = $inventaris->groupBy(function ($item) {
            return $item->display_nama_barang;
        });

        $inventarisGrouped = $grouped->map(function ($items, $nama) {
            return [
                'nama_barang' => $nama,
                'total_keseluruhan' => $items->sum('jumlah'),
                'items' => $items->values(),
    ...
    public function show(Inventaris $inventari)
{
    // Memuat relasi yang diperlukan untuk item spesifik ini
    $inventari->load(['masterBarang', 'ruangan', 'asalPerolehan', 'user', 'jenisPerawatan']);
...
{
    return Inertia::render('Inventaris/Form', [
        'ruanganList' => MasterRuangan::all()->map(fn ($r) => [
            'id' => $r->id,
            'label' => $r->nomor_ruang
                ? "{$r->nama_ruangan} ({$r->nomor_ruang})"
                : $r->nama_ruangan,
        ]),
        'asalPerolehanList' => MasterAsalPerolehan::all()->map(fn($a) => ['id' => $a->id, 'label' => $a->nama]),
        'masterBarangList' => MasterBarang::with('kategori')->get()->map(fn ($b) => [
            'id' => $b->id,
            'label' => $b->nama_barang,
            'kategori_id' => $b->master_kategori_barang_id,
        ]),
        'kategoriList' => MasterKategoriBarang::all()->map(fn($k) => ['id' => $k->id, 'label' => $k->nama]),
        'jenisPerawatanList' => MasterJenisPerawatan::all()->map(fn($j) => ['id' => $j->id, 'label' => $j->nama]),
    ]);
}
...
public function store(Request $request)
{
    ...
    Log::info('--- PROSES STORE INVENTARIS DIMULAI ---');
    Log::info('Data Request Masuk:', $request->all());
    // dd($request->all());
    // Siapkan data untuk validasi, konversi master_kategori_id ke master_kategori_barang_id
    $dataToValidate = $request->all();
    if (isset($dataToValidate['master_kategori_id']) && !isset($dataToValidate['master_kategori_barang_id'])) {
        $dataToValidate['master_kategori_barang_id'] = $dataToValidate['master_kategori_id'];
        unset($dataToValidate['master_kategori_id']);
    }

    $validator = Validator::make($dataToValidate, [
        // master_barang_id sekarang WAJIB karena nama_barang tidak disimpan di Inventaris
        'master_barang_id'          => 'required|exists:master_barang,id',
        'master_ruangan_id'         => 'nullable|exists:master_ruangan,id',
        'master_asal_perolehan_id'  => 'nullable|exists:master_asal_perolehan,id',
        'master_jenis_perawatan_id' => 'nullable|exists:master_jenis_perawatan,id',
    ...
    Log::info('Validasi Berhasil. Data tervalidasi (sebelum proses):', $validated);

    try {
        DB::transaction(function () use ($request, &$validated) {
            // Set User ID dan Nama Pengunggah
            $validated['uploaded_by'] = Auth::user()->id;
            // $validated['nama_pengunggah'] = Auth::user()->name;
            // Log::info('User ID Pelapor:', ['id' => $validated['uploaded_by'], 'name' => $validated['nama_pengunggah']]);

            // Ambil MasterBarang yang dipilih
            $masterBarang = MasterBarang::find($validated['master_barang_id']);
            if (!$masterBarang) {
                // Ini seharusnya tertangkap oleh validasi 'exists', tapi sebagai fallback
                throw new \Exception('Master barang tidak ditemukan.');
            }

            // master_kategori_barang_id akan selalu diambil dari MasterBarang yang dipilih
            // Ini memastikan Inventaris selalu memiliki kategori yang sesuai dengan MasterBarang-nya

            // Proses Upload Foto ke Supabase
            if ($request->hasFile('foto')) {
                Log::info('File foto terdeteksi, memulai upload ke Supabase...');

                $file = $request->file('foto');
                $originalFileName = pathinfo($file->getClientOriginalName(), PATHINFO_FILENAME);
                $extension = $file->getClientOriginalExtension();
                $fileName = time() . '_' . Str::slug($originalFileName) . '.' . $extension;
                ...
                if ($response->successful()) {
                    $validated['foto'] = env('SUPABASE_URL') . '/storage/v1/object/public/inventaris-fotos/' . $fileName;
                    Log::info('Upload Supabase Berhasil:', ['url' => $validated['foto']]);
                } else {
                    Log::error('Upload Supabase GAGAL:', [
                        'status' => $response->status(),
                        'body'   => $response->body()
                    ]);
                    throw new \Exception('Gagal upload foto ke storage: ' . $response->body());
                }
            }

            // Log Payload Final sebelum Insert ke Database
            Log::info('Payload Final sebelum Create (DB):', $validated);

            // Eksekusi Create
            $record = Inventaris::create($validated);

            Log::info('Data Berhasil Disimpan ke Database dengan ID: ' . $record->id);
        });

        Log::info('--- TRANSAKSI SELESAI (SUCCESS) ---');
        return redirect()->route('inventaris.index')->with('success', 'Data berhasil ditambahkan.');

    } catch (\Exception $e) {
        Log::error('--- TRANSAKSI GAGAL (EXCEPTION) ---');
    ...
    private function determineAndValidateId(string $nameValue, string $idColumn, string $nameColumn, ?int $ignoreRecordId = null): int
    {
        // Cari apakah nama barang ini sudah punya nomor
        $existingGroup = Inventaris::where($nameColumn, $nameValue)->whereNotNull($idColumn)->first();
        
        if ($existingGroup) {
            $finalId = $existingGroup->$idColumn;
        } else {
            // Jika belum ada, cari nomor ID (integer) yang masih kosong
            $allIds = Inventaris::query()->whereNotNull($idColumn)->distinct()->pluck($idColumn)->sort();
            $finalId = 1;
            foreach ($allIds as $id) {
                if ($finalId < $id) break;
...
public function edit(Inventaris $inventari): InertiaResponse
    {
        // Pastikan relasi masterBarang dimuat untuk mengisi form
        $inventari->load('masterBarang');

        return Inertia::render('Inventaris/Form', [
            'inventari' => $inventari,
            // Kirim master_barang_id secara eksplisit jika perlu, meski biasanya sudah ada di object inventari
            // Frontend harus menangani logic: jika inventari ada, set default master_barang_id dari inventari.master_barang_id
    ...
    {
        Log::info('--- PROSES UPDATE INVENTARIS DIMULAI ---');
        Log::info('Data Request Update Masuk:', $request->all());

        // Siapkan data untuk validasi (sama seperti store)
        $dataToValidate = $request->all();
        // Hapus manipulasi master_kategori karena sekarang otomatis diambil dari master_barang
        ...
        $validator = Validator::make($dataToValidate, [
            ...
            'master_asal_perolehan_id'  => 'nullable|exists:master_asal_perolehan,id',
            'master_jenis_perawatan_id' => 'nullable|exists:master_jenis_perawatan,id',
            'jumlah'                    => 'required|integer|min:1', // Jumlah biasanya required saat update
            'tanggal_masuk'             => 'required|date',
            'spesifikasi'               => 'nullable|string',
            'jumlah_dipakai'            => 'nullable|integer|min:0',
            'jumlah_rusak'              => 'nullable|integer|min:0',
        ...
        if ($validator->fails()) {
            Log::error('Validasi Update Gagal:', $validator->errors()->toArray());
            return redirect()->back()->withErrors($validator)->withInput();
        }

        $validated = $validator->validated();
        // Hapus foto dari array validated agar tidak menimpa jika tidak ada upload baru
        // Kita akan handle manual di bawah
        ...
        try {
            ...
            // Logika Hapus Foto Lama (jika diminta user)
            if ($request->boolean('remove_foto') && $inventari->foto) {
            ...
            if ($request->hasFile('foto')) {
                // Hapus foto lama dulu jika ada (untuk menghemat storage)
                if ($inventari->foto) {
                    $oldPath = str_replace(env('SUPABASE_URL') . '/storage/v1/object/public/', '', $inventari->foto);
                    Http::withHeaders([
                        'apikey' => env('SUPABASE_KEY'),
                ...
                if ($response->successful()) {
                    ...
                    throw new \Exception('Gagal upload foto baru ke Supabase: ' . $response->body());
                }
            }

            DB::transaction(function () use ($validated, $inventari) {
                // Pastikan master_kategori_barang_id sinkron dengan master_barang yang baru (jika berubah)
                if (isset($validated['master_barang_id'])) {
                     $masterBarang = MasterBarang::find($validated['master_barang_id']);
                }

                // Update info pengunggah terakhir (opsional, jika ingin track siapa yang update)
                // $validated['uploaded_by'] = Auth::id(); // Uncomment jika ingin mengubah owner saat update
            ...
            Log::error('Update Inventaris Error: ' . $e->getMessage());
    ...
    public function bulkEdit(Request $request)
{
    ...
    $inventaris = Inventaris::whereIn('id', $ids)->get();

    return Inertia::render('Inventaris/BulkEditForm', [
        'inventaris' => $inventaris,
        // Kirim list master untuk dropdown di React
        'ruanganList' => MasterRuangan::all()->map(fn ($r) => [
            'id' => $r->id,
            'label' => $r->nomor_ruang
                ? "{$r->nama_ruangan} ({$r->nomor_ruang})"
                ...
                'asalPerolehanList' => MasterAsalPerolehan::all()->map(fn($a) => ['id' => $a->id, 'label' => $a->nama]),
        'masterBarangList' => MasterBarang::all()->map(fn($b) => [
            'id' => $b->id, 
            'label' => $b->nama_barang,
            'kategori_id' => $b->master_kategori_barang_id
        ]),
    ...
    public function bulkUpdate(Request $request)
{
    $validated = $request->validate([
        'items' => 'required|array',
        'items.*.id' => 'required|exists:inventaris,id',
        // Update validasi ke Master ID
        'items.*.master_barang_id' => 'required|exists:master_barang,id',
        'items.*.master_ruangan_id' => 'required|exists:master_ruangan,id',
        'items.*.master_asal_perolehan_id' => 'required|exists:master_asal_perolehan,id',
        'items.*.master_jenis_perawatan_id' => 'nullable|exists:master_jenis_perawatan,id',
        'items.*.master_kategori_id' => 'nullable|exists:master_kategori_barang,id',
    ...
    try {
        DB::transaction(function () use ($validated, $request) {
            foreach ($validated['items'] as $index => $itemData) {
                ...
                // --- LOGIKA FOTO SUPABASE (Tetap Sama) ---
                // --- FOTO ---
                if ($request->hasFile("items.{$index}.foto")) {
                    // hapus foto lama di Supabase
                    if ($inventari->foto) {
                ...
                // Tambahkan pengunggah
                $itemData['nama_pengunggah'] = Auth::user()->name;

                // Update Nama Barang berdasarkan Master Barang jika diperlukan (opsional)
                $master = MasterBarang::find($itemData['master_barang_id']);
                $itemData['nama_barang'] = $master->nama_barang;

                $inventari->update($itemData);
            }
        });

        return redirect()->route('inventaris.index')->with('success', 'Data massal berhasil diperbarui.');
    } catch (\Exception $e) {
        Log::error('Bulk update gagal: ' . $e->getMessage());
        return redirect()->back()->with('error', 'Gagal memperbarui data: ' . $e->getMessage());
    }
    ...
    protected function drawLabel(Fpdf $fpdf, Inventaris $item, float $x, float $y)
{
    ...
    $fpdf->Cell($infoCellW, $titleBoxH, 'PENGADAAN BARANG', 0, 0, 'C');
     ...
     // Ganti nomor pengadaan
     $nomorPengadaan = $item->nomor_pengadaan_lengkap ?? ($item->kode_barang ?? 'N/A');
     // Ganti unit pengguna
     $unitPengguna = $item->display_ruangan;
    ...
    $kelompokTextHeight = 4;
    $namaBarang = $item->display_nama_barang;
    $rawSpek = $item->spesifikasi ?: '...';
    ...
    public function exportExcel()
    {
        ...
        . '/storage/v1/object/public/inventaris-fotos/aset/logo_klinik.png';
            $tempPath = sys_get_temp_dir() . '/logo_klinik.png';
        ...
        $sortedData = $data->sortBy(function ($item) {
            return $item->display_nomor_barang;
        });
        ...
        foreach ($sortedData as $item) {
            ...
            $sheet->setCellValue('C' . $row, $item->display_nomor_barang ?? 'N/A');
            $sheet->setCellValue('D' . $row, $item->display_nama_barang ?? 'N/A');
            $sheet->setCellValue('E' . $row, $item->display_kode_barang ?? 'N/A');
            $sheet->setCellValue('F' . $row, $item->spesifikasi ?? '-');
            $sheet->setCellValue('G' . $row, $item->display_jenis_perawatan ?? '-');
        
            $sheet->setCellValue('H' . $row, $item->jumlah ?? 0);
            $sheet->setCellValue('I' . $row, $item->jumlah_dipakai ?? 0);
            $sheet->setCellValue('J' . $row, $item->jumlah_rusak ?? 0);
        
            $sheet->setCellValue('K' . $row, $item->display_ruangan ?? '-');
            $sheet->setCellValue('L' . $row, $item->display_nomor_ruang ?? '-');
            $sheet->setCellValue('M' . $row, $item->display_asal ?? '-');
        
            $sheet->setCellValue(
                'N' . $row,
                optional($item->tanggal_masuk)->format('Y-m-d')
            );
    ...
    public function print()
    {
        ...
        ->leftJoin('master_barang', 'inventaris.master_barang_id', '=', 'master_barang.id')
        ->orderBy('master_barang.nomor_barang', 'asc')
        ->select('inventaris.*')
        ->get();
        ...
        // --- HEADER DOKUMEN ---
        try {
            $logoUrl = rtrim(config('services.supabase.url'), '/') . '/storage/v1/object/public/inventaris-fotos/aset/logo_klinik.png';
            $imageContents = Http::timeout(10)->get($logoUrl)->body();
            $tempPath = tempnam(sys_get_temp_dir(), 'logo') . '.png';
            file_put_contents($tempPath, $imageContents);
            $this->fpdf->Image($tempPath, 10, 8, 25);
        ...
        foreach ($inventaris as $i => $item) {
            $data = [
                $i + 1,
                '',
                $item->display_nomor_barang ?? 'N/A',
                $item->display_nama_barang ?? 'N/A',
                $item->display_kode_barang ?? 'N/A',
                $item->spesifikasi ?? '-',
                $item->display_jenis_perawatan ?? '-',
                $item->jumlah ?? 0,
                $item->jumlah_dipakai ?? 0,
                $item->jumlah_rusak ?? 0,
                $item->display_ruangan ?? '-',
                $item->display_nomor_ruang ?? '-',
                $item->display_asal ?? '-',
                $item->tanggal_masuk
    ...
    public function exportPDF(Request $request)
    {
        ...
        if ($request->filled('tanggal_mulai')) {
            ...
            // jika tanggal_selesai kosong → pakai hari ini
            ...
            $titleParts[] = sprintf(
                'Periode %s - %s',
                $start->isoFormat('D MMMM Y'),
                $end->isoFormat('D MMMM Y')
            );
        }
         else {
            if ($request->filled('hari')) {
        ...
        $inventaris = $query
        ->orderBy(
            MasterBarang::select('nomor_barang')
                ->whereColumn('master_barang.id', 'inventaris.master_barang_id')
        )
        ->get();    
